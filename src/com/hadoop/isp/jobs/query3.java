 ...
 public static class Map extends Mapper<LongWritable, Text, IntWritable, Text> {
    ...
    public void map(LongWritable key, Text value, OutputCollector<IntWritable, Text> output, Reporter reporter) throws IOException {
		...
		if(filename.equals("customers")) {
			...
			transNum = Integer.parseInt(splits[3]);
			transTotal = Float.parseFloat(splits[2]);
			minItems = Integer.parseInt(splits[3]);
			output.collect(customerID.set(ID), transactionInfo.set(tag + "," + String.valueOf(ID) + "," + String.valueOf(transNum) + "," + String.valueOf(transTotal) + "," + String.valueOf(minItems)))
		}
    }
 } 
	...
	public void reduce(IntWritable key, Iterator<Text> values, OutputCollector<IntWritable, Text> output, Reporter reporter)throws IOException {
		...
		while (values.hasNext()) {
			...
			if(splits[0].equals("Customer")){
				...
				if(!custTransInfo.containsKey(key.get())) {
					custTransInfo.put(key.get(), splits[1] + "," + "" + "," + "" + "," + splits[2] + "," + splits[3] + "," + splits[4]);
				}
				else{
					String[] current = custTransInfo.get(key.get()).split(",");
					final int currentTransNum = Integer.parseInt(current[3]);
					...
					custTransInfo.put(key.get(), current[0] + "," + current[0] + "," + current[0] + "," + String.valueOf(currentTransNum + Integer.parseInt(splits[2])) 
										+ "," + String.valueOf(currentTransTotal + Integer.parseInt(splits[3])) + "," + minItems);
				}
			}
		}
